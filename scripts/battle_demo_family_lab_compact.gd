...
# Compact presentation for the family-based combat lab.
# Family logic stays unchanged; this layer only prevents long labels such as
# "Schiggy-Familie -> Schillok" from overflowing the setup panels.
#
# Each row now reads compactly as:
#   Schiggy   [Schillok]   Lv.17
# or, before evolution:
#   Schiggy   [Basis]      Lv.10
...
func _fill_rows(box: VBoxContainer, setup: Array, own: bool) -> void:
    ...
    for index: int in range(mini(box.get_child_count(), setup.size())):
        ...
        # Keep the family chooser short. Metadata still stores the canonical
        # family root, so only presentation changes here.
        ...
        var badge: Control = _make_form_badge(family_id, resolved_id, level)
        row.add_child(badge)
...
func _make_form_badge(family_id: String, resolved_id: String, level: int) -> Control:
    var badge := PanelContainer.new()
    badge.custom_minimum_size = Vector2(72.0, 22.0)
