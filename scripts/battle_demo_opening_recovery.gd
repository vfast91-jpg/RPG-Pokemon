...
# - The default debt is 35 ATB points; individual moves may override it with
#   `opening_atb_debt` in their data.

const DEFAULT_PRIORITY_OPENING_ATB_DEBT: float = 35.0
...
func _opening_debt_for_move(move: Dictionary) -> float:
    ...
    if int(move.get("priority", 0)) > 0:
        return DEFAULT_PRIORITY_OPENING_ATB_DEBT
    return 0.0
