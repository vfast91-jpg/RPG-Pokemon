...
# Append-only roster extension for the final Gen-2 family: Celebi.
# This layer chains after families 41-50 and adds Celebi to the same
# generation-neutral runtime/canonical Pokemon dictionaries.

const GEN2_51_EXTENSION_MANIFEST_PATH: String = "res://data/pokemon_database_extension_51_v1.json"


func _load_data() -> void:
	super._load_data()
	_gen2_51_load_roster_extension()


func _gen2_51_load_roster_extension() -> void:
	if not pokemon_registry_ready():
		return

	var extension: Dictionary = _remaining_read_json_dictionary(GEN2_51_EXTENSION_MANIFEST_PATH)
	if extension.is_empty():
		_gen2_51_fail("Gen2-51-Erweiterungsmanifest fehlt.")
		return
	...
	if not (runtime_species_value is Dictionary and canonical_species_value is Dictionary):
		_gen2_51_fail("Basis-Pokemonpool ist vor der Celebi-Erweiterung ungültig.")
		return
	if not (roots_value is Array and family_members_value is Dictionary):
		_gen2_51_fail("Basis-Familiengraph ist vor der Celebi-Erweiterung ungültig.")
		return
	...
	if runtime_species.size() != expected_base_species or roots.size() != expected_base_roots:
		_gen2_51_fail("Celebi-Erweiterung passt nicht zum aktiven Gen2-01-50-Roster.")
		return
	...
	if not (core_value is Dictionary and new_roots_value is Array and new_families_value is Dictionary):
		_gen2_51_fail("Celebi-Erweiterungsdaten sind ungültig.")
		return
	...
	if detail_files_value is Array:
		for path_value: Variant in detail_files_value:
			...
			if not (entries_value is Dictionary):
				_gen2_51_fail("Celebi-Detailpaket ist ungültig: " + str(path_value))
				return
			for species_id_value: Variant in (entries_value as Dictionary).keys():
				...
				if details_by_species.has(species_id):
					_gen2_51_fail("Doppelte Pokemon-ID in Celebi-Detailpaketen: " + species_id)
					return
	...
	if core_species.size() != int(extension.get("extension_species_count", -1)):
		_gen2_51_fail("Celebi-Core enthält eine falsche Pokemon-Anzahl.")
		return
	if details_by_species.size() != core_species.size():
		_gen2_51_fail("Celebi-Detailpakete decken den Core nicht vollständig ab.")
		return

	for species_id_value: Variant in core_species.keys():
		var species_id: String = str(species_id_value)
		if canonical_species.has(species_id):
			_gen2_51_fail("Celebi-Erweiterung kollidiert mit bestehender Pokemon-ID: " + species_id)
			return
		var core_entry_value: Variant = core_species.get(species_id_value, {})
		if not (core_entry_value is Dictionary):
			_gen2_51_fail("Ungültiger Celebi-Core-Datensatz: " + species_id)
			return
		...
		if str(core_entry.get("species_id", "")) != species_id:
			_gen2_51_fail("Widersprüchliche Celebi-Core-ID: " + species_id)
			return
	...
	for root_value: Variant in (new_roots_value as Array):
		...
		if roots.has(root_id) or family_members.has(root_id):
			_gen2_51_fail("Doppelte Celebi-Familienwurzel: " + root_id)
			return
		...
		if not (members_value is Array) or (members_value as Array).is_empty():
			_gen2_51_fail("Leere Celebi-Familie: " + root_id)
			return
		for member_value: Variant in (members_value as Array):
			if not runtime_species.has(str(member_value)):
				_gen2_51_fail("Celebi-Familienmitglied fehlt im Runtime-Pool: " + str(member_value))
				return
	...
	if runtime_species.size() != int(extension.get("runtime_species_count", -1)):
		_gen2_51_fail("Celebi-Runtime-Pokemonanzahl ist unvollständig.")
		return
	if roots.size() != int(extension.get("runtime_route_root_count", -1)):
		_gen2_51_fail("Celebi-Runtime-Familienanzahl ist unvollständig.")
		return
	...
	print("Gen2 final family 51 extension OK: %d Pokemon, %d families total" % [runtime_species.size(), species_ids.size()])


func _gen2_51_fail(message: String) -> void:
